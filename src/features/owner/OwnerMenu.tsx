...
export function OwnerMenuPage() {
  return <><PageHeader title="Más opciones" description="Las funciones del edificio, agrupadas para que encuentres cada cosa cuando la necesites." /><div className="owner-menu-groups">{groups.map((group, index) => <details className="owner-menu-group" key={group.title} open={index === 0}><summary><span><span className="owner-menu-summary-kicker">Sección</span><strong>{group.title}</strong></span><span className="owner-menu-count">{group.items.length} opciones<ChevronRight size={18} /></span></summary><div className="owner-menu-list">{group.items.map(({ to, title, description, Icon }) => <Link className="owner-menu-row" to={to} key={to}><span className="owner-menu-icon"><Icon size={21} /></span><span className="owner-menu-copy"><strong>{title}</strong><small>{description}</small></span><ChevronRight size={20} /></Link>)}</div></details>)}</div><p className="owner-menu-demo-note"><ArrowUpRight size={17} />Esta es una demostración. Las acciones se guardan solo en este navegador.</p></>;
}
